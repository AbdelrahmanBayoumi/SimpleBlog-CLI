...
if ARGV[0] == "--help" then
    ...
    For Posts   
    Create new post             --creaetpost <title> <body, separated by _> <tags, separated by ,>
    Update post data            --updatepost <post_id> <title> <body> <tags>
    Get post data               --postdata <post_id>
    Get all posts data          --postsdata
    For Comments
    Create comment for posts    --addcomment <post_id> <body>
    Update comment data         --updatecomment <comment_id> <body>
    For Followers
...
# User
    ...
    # ruby client.rb --signup ali@example.com mahmoud@ALI98 mahmoud@ALI98  ali saad 01232134121
    ...
    # ruby client.rb --signin ali@example.com mahmoud@ALI98
    user = User.new(email: ARGV[1], password: ARGV[2])
    res = user.sign_in
    puts res
elsif ARGV[0] == "--userdata" then
    # ruby client.rb --userdata 2
    user = User.new
    ...
    # ruby client.rb --updateuser 2 ali@example.com omar saad 01232134121 male
...
# Post
    ...
    post = Post.new(title: ARGV[1], body: ARGV[2].gsub('_',' '), all_tags: ARGV[3])
    res = post.creaet_post(post)
    puts res
elsif ARGV[0] == "--updatepost" then
    # ruby client.rb --updatepost 2 test2 my_name_is_omar we,orange
    post = Post.new(title: ARGV[2], body: ARGV[3].gsub('_',' '), all_tags: ARGV[4])
...
# Comment
    ...
    post = Post.new
    res = post.add_comment(ARGV[2].gsub('_',' '), ARGV[1])
    puts res
